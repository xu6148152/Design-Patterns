 ...
 * with variable
 * durations and then continues its own work. When the main thread has done it's job it collects
 * the
 * results of the
 * async tasks. Two of the tasks are handled with callbacks, meaning the callbacks are executed
 * immediately when the
 * tasks complete.
 * <p>
 * Noteworthy difference of thread usage between the async results and callbacks is that the async
 * results are collected
 * in the main thread but the callbacks are executed within the worker threads. This should be
 * noted
 * when working with
 * thread pools.
 * <p>
 ...
 * ExecutorService are the real world implementations of this pattern. But due to the nature of
 * parallel programming,
 * the implementations are not trivial. This example does not take all possible scenarios into
 * account but rather
...
public class App {
    ...
    //    System.out.println(
    //            String.format("[%1$-10s] - %2$s", Thread.currentThread().getName(), msg));
    //}
}
